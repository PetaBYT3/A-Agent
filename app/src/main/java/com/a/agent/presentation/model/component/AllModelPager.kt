...
fun AllModelPager(
    ...
    LazyColumn(
        ...
        when {
            state.isAllModelsLoading -> {
                item(key = "isAllModelsLoading") {
                    LinearWavyProgressIndicator(
                        modifier = modelPagerMessagePadding()
                    )
                }
            }
            state.isAllModelsError != null -> {
                item(key = "isAllModelsError") {
                    Message(
                        modifier = modelPagerMessagePadding(),
                        message = state.isAllModelsError,
                        messageType = MessageType.Error
                    )
                }
            }
            state.allModels.isEmpty() -> {
