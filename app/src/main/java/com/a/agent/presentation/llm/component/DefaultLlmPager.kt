...
fun DefaultLlmPager(
    ...
    LazyColumn(
        ...
        when {
            state.isDefaultLlmLoading -> {
                item(key = "isDefaultLlmLoading") {
                    LinearWavyProgressIndicator(
                        modifier = pagerItemModifier
                    )
                }
            }
            state.isDefaultLlmError != null -> {
                item(key = "isDefaultLlmError") {
                    Message(
                        modifier = pagerItemModifier,
                        message = state.isDefaultLlmError,
                        messageType = MessageType.Error
                    )
                }
            }
            state.defaultLlm.isEmpty() -> {
                item(key = "isDefaultLlmEmpty") {
                    Message(
                        modifier = pagerItemModifier,
                        message = "Model Empty",
